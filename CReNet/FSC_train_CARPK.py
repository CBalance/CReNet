...
def get_args_parser():
    ...
    # Dataset parameters
    parser.add_argument('--data_path', default='/data/carpk', type=str,
                        help='dataset path')
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for data_iter_step, data in enumerate(metric_logger.log_every(dataloader_train, print_freq, header)):
            ...
            for i in range(output.shape[0]):
                ...
                if i == 0 and data_iter_step % 100 == 0:
        ...
        log_stats = {**{f'train_{k}': v for k, v in train_stats.items()},
                        'Current MAE': train_mae/(len(dataloader_train)),
                        'RMSE':  (train_rmse/(len(dataloader_train)))**0.5,
                        'epoch': epoch,}
        print('Current MAE: {:5.2f}, RMSE: {:5.2f} '.format( train_mae/(len(dataloader_train)), (train_rmse/(len(dataloader_train)))**0.5))
        ...
        if args.output_dir and train_mae/(len(dataloader_train)) < min_MAE:
            min_MAE = train_mae/(len(dataloader_train))
            misc.save_model(
                args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                loss_scaler=loss_scaler, epoch=epoch, suffix="carpk_minMAE")
