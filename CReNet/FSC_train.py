...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default='./data/out',
    ...
    parser.add_argument("--title", default="CReNet", type=str)
...
def main(args):
    ...
    dataset_eval = EvalData(args.external, args.box_bound)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if True:
            for data_iter_step, (samples, gt_density, boxes, m_flag) in enumerate(
                ...
                for i in range(output.shape[0]):
                    ...
                    if data_iter_step % 100 == 0 and i == 0:
        ...
        for data_iter_step, (samples, gt_dots, boxes, pos, gt_map, im_name) in \
                enumerate(metric_logger.log_every(data_loader_eval, print_freq, header)):
            ...
            if args.normalization:
                ...
                e_cnt = e_cnt / 3
                if e_cnt > 1.8 or 0.45 < e_cnt < 1:
                    pred_cnt /= e_cnt
                else:
                    pred_cnt = pred_cnt - e_cnt * 3 + 3
            ...
            if data_iter_step % 100 == 0:
                print(
                f'{data_iter_step}/{len(data_loader_eval)}: pred_cnt: {pred_cnt},  gt_cnt: {gt_cnt},  error: {cnt_err},  AE: {cnt_err},  SE: {cnt_err ** 2}, id: {im_name.name}, s_cnt: {s_cnt >= 1}')
        ...
        log_stats = {'eval_MAE': eval_mae / (len(data_loader_eval)),
                     'eval_RMSE': (eval_rmse / (len(data_loader_eval))) ** 0.5}
        ...
        print('Current MAE: {:5.2f}, RMSE: {:5.2f}, min_MAE: {:5.2f} '.format(eval_mae / (len(data_loader_eval)), (
                eval_rmse / (len(data_loader_eval))) ** 0.5, min_MAE))

        eval_mae = eval_mae / len(data_loader_eval)
        eval_rmse = eval_rmse / len(data_loader_eval)
        if args.output_dir and eval_mae < min_MAE:
            min_MAE = eval_mae
            misc.save_model(
                args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                loss_scaler=loss_scaler, epoch=epoch, suffix="eval_minMAE")
